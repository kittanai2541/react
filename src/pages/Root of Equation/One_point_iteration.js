...
class Onepoint extends Component {
    ...
    db = async()=>{
        var response = await axios.get('http://localhost:3001/api/users/showonepointt').then(res => {return res.data});
        this.setState({
    ...
    render() {
        return (
            <div style={{ background: "#FFFF", padding: "30px" }}>
                <h2 style={{ color: "black", fontWeight: "bold",marginLeft:"44%" }}>Onepoint</h2>
                <div>
                    {!this.state.showOutputCard && <Card
                        title={"Input onepoint"}
                        bordered={true}
