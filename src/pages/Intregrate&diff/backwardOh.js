...
class Backwardh extends Component {
    ...
    backwardh(degree,x0,h) {
        switch (degree) {
            case 1:
                y = (this.func(x0) - this.func(x0-(1*h))) / h
                break;
            case 2:
                y = (this.func(x0) - 2*this.func(x0-(1*h)) + this.func(x0-(2*h))) / Math.pow(h, 2)
                break;
            case 3:
                y = (this.func(x0) - 3*this.func(x0-(1*h)) + 3*this.func(x0-(2*h)) - this.func(x0-(3*h))) / Math.pow(h, 3)
                break;
            default:
                y = (this.func(x0) - 4*this.func(x0-(1*h)) + 6*this.func(x0-(2*h)) - 4*this.func(x0-(3*h)) + this.func(x0-(4*h))) / Math.pow(h, 4) 
        }
        this.setState({
            showOutputCard: true
        })
    ...
    handleChange(event) {
     ...
     db = async()=>{
        ...
        this.backwardh(this.state.degree,this.state.x0,this.state.h);
    } 
    render() {
        return (
            <div style={{ background: "#FFFF", padding: "30px" }}>
                <h2 style={{ color: "black", fontWeight: "bold",marginLeft:"44%" }}>BackwardOH</h2>
                <div>
                    ...
                    >
                        ...
                        <Button id="submit_button" onClick={
                            () => this.backwardh(parseFloat(this.state.degree),parseFloat(this.state.x0),parseFloat(this.state.h))
                        }
